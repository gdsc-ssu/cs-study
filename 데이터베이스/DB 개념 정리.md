...
## 데이터베이스란?
- 여러 사람의 공유 목적으로 체계화해 통합, 관리되는 데이터의 집합이다.
- 데이터 베이스를 사용하기 이전 : 파일 시스템을 이용해 관리한다.     
    → 데이터 종속성, 중복성, 무결성의 문제가 발생한다.     
...
- **물리적 독립성** : 응용 프로그램과 물리적 장치(ex. 보조기억장치 etc.)간의 독립
    - 데이터베이스 시스템의 성능 향상을 위한 새로운 디스크의 도입에도 응용 프로그램에 영향이 없다.
    - 데이터의 **물리적 구조**만을 변경한다,
    - 데이터베이스의 사이즈 변경 시 관련된 응용 프로그램의 수정이 필요 없다.

- cf) **종속성**(Data Dependency) : 응용 프로그램의 구조가 데이터의 구조에 영향을 받는다.

### 2️⃣ 무결성 (Data Integrity)
- 데이터의 삽입, 삭제, 갱신 등의 연산 후에도 데이터 베이스에 저장되어있는 데이터가 **정해진 제약조건을 항상 만족**해야 한다.
- 여러 경로를 통해 잘못된 데이터가 발생하는 경우를 방지하기 위한 목적을 가진다.
- 데이터의 유효성을 검사한다.

### 3️⃣ 일관성 (Data Consistency)
- 데이터베이스에 저장되어있는 데이터와 특정 질의에 대한 응답이 변함 없이 일정해야 하는 조건이다.
- 관련있는 정보를 논리적인 구조로 관리하는 체계이다.
- 데이터 변경 시 나머지 데이터와 일치하지 않는 **불일치성의 배제**를 위한 목적을 가진다.
- 유효한 데이터에 대한 데이터베이스의 자체 규칙을 위반하는 데이터는 사용할 수 없다고 명시한다.
    - ex) 성별 column에는 'Male', 'Female', 'Unknown'만 들어갈 수 있다
- 일치하지 않는 데이터를 도입하려는 특정 트랜잭션이 발생할 경우, 전체 트랜잭션이 롤백되고 오류가 사용자에게 반환된다.

### 4️⃣ 보안성 (Data Security)
- 데이터로의 접근 권한이 있는 사용자들만 접근할 수 있도록 설정한다.

### 5️⃣ 중복성 (Data Redundancy)
- 같은 데이터를 중복으로 저장하는 행위이다.
- 데이터의 수정, 삭제가 필요할 경우 연결되어있는 모든 데이터를 수정 및 삭제해줘야 하는 문제점이 존재한다.
- **데이터의 통합 관리**를 통해 데이터의 중복을 최소화하는 것을 지향한다.
...
- DBMS(DataBase Management System)은 **항상 정렬된 상태를 유지**한다.
    - 탐색에 효율적
    - 값의 추가, 삭제, 수정 비효율적
...
- `B-Tree` : 컬럼 값의 변형 없이 원래의 값을 이용해 인덱싱 하는 방법(일반적으로 사용되는 방법)
    ...
    - 데이터 검색 속도가 일정하다.
    - 균형 트리
    - 브랜치 노드에 key, data를 담을 수 있다.
<img width="605" src="https://user-images.githubusercontent.com/66112716/200322592-d7125ccd-ccd5-4bec-a32f-9df34101d902.png">

- `Hash` : 컬럼의 값으로 해시 값을 계산해 인덱싱하는 방법
    - 매우 빠른 검색 속도를 가진다.
    - 값을 변형해 인덱싱하기에 값의 일부만으로 검색할 때에는 사용할 수 없다.
    - `B-Tree`보다 `Hash`가 효율적일 것 같으나 `SELECT`의 질의 조건에 부등호(< , >)도 포함된다.    
...
- 데이터 관리 목적으로 보안, 무결성, 회복, 병행 제어 등을 정의할 때 사용된다.
...
- 데이터베이스 작업의 완전성을 보장해주기 위한 수단이다.
- 모두 완벽하게 처리되거나 처리하지 못할 경우 원상태로 복구해 작업의 일부만이 적용되는 현상을 방지하기 위한 기능이다.

### ▶️ 트랜잭션의 특성 - ACID
- `Atomicity` (원자성) : 트랜잭션 중간에 어떤 문제가 발생하였을 때 아래의 상황에 따라 작업 내용의 반영 여부가 달라진다.
    - 모두 성공할 경우 : 모두 반영된다.
    - 하나라도 실패한 경우 : 모두 반영되지 않는다.
- `Consistency` (일관성) : 트랜잭션의 처리 결과는 항상 데이터의 일관성을 보장해야 한다.
- `Isolation` (고립성) : 둘 이상의 트랜잭션이 동시에 실행되고 있을 경우, 각 트랜잭션은 서로의 간섭 없이 독립적으로 수행된다.
- `Durability` (지속성) : 트랜잭션이 정상적으로 종료된 뒤 영구적으로 데이터베이스 작업의 결과가 저장되어야 한다.
...
### ▶️ 교착상태 발생의 필요충분 조건
- **상호배제** : 한 번에 한 개의 프로세스만 자원을 사용할 수 있어야 한다.
- **점유와 대기** : 프로세스가 한개의 자원을 점유하고 있고, 추가적으로 다른 프로세스의 자원을 점유하기 위해서는 대기해야 한다.
- **비선점** : 점유된 자원을 강제로 해제될 수 없고, 프로세스가 자원의 사용을 자발적으로 해제하기 전까지 그 자원을 얻을 수 없다.
- **환형 대기** : 자원과 자원을 사용하기 위해 대기하는 프로세스들이 원형으로 구성되어 있어 자신에게 할당된 자원을 점유하여 앞 또는 뒤에 있는 프로세스의 자원을 요구해야 한다.
